...
fn read_line( text: &str ) -> String {
    ...
    print!( "{}", text );
    io::stdout().flush().ok().expect( "Не удалось очистить stdout!");
    stdin.lock().read_line( &mut buffer ).ok().expect( "Невозможно прочитать строку!" );
...
fn read_file( filename: String ) -> Vec<u8> {
    ...
    let result = file.read_to_end( &mut code ).unwrap();
    code
}
...
fn main() {
    ...
    while code_index < code.len() {
        match code[code_index] as char {
            ...
            ',' => {
                let stdin = io::stdin();
                memory[cell_index] = stdin.lock().bytes().next().unwrap().unwrap() as i64;
            },
